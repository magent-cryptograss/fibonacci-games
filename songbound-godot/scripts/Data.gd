...
func enemy_damage(atk: float, def_val: float, rng: RandomNumberGenerator = null) -> int:
	return mitigate(atk * 1.55, def_val, rng)
...
func make_enemy(id: String, tier: float) -> Dictionary:
	...
	var hp_mul := 1.0 if is_boss else pow(tier, 1.6)
	...
	return {
		...
		"skills": b.skills,
		"maxhp": hp, "hp": hp,
		"atk": roundi(float(b.atk) * t), "def": roundi(float(b.def) * t),
